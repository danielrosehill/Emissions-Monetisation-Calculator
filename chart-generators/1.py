...
French Government - Quinet Report (2019-01-01),French Government,The government of France.,2019-01-01,France,FRA,FR,0.91,Very High,"Recommended range for the SCC with a central value around 250 Euros per tonne CO2e by 2030, rising steeply after that.",€250-€770,Euros,EUR,510,555.14,555140000,2024-11-20,euros per ton CO2e,tons of carbon dioxide equivalents,SCC range with projected increase,Social Cost of Carbon,1,
...
# Sort DataFrame by date in descending order (newest to oldest)
df = df.sort_values(by='date', ascending=False)

# Create the bar plot
plt.figure(figsize=(12, 6))  # Adjust figure size for better readability
plt.bar(df['proposal_with_date'], df['usd_proposed_value'])

# Rotate x-axis labels for better readability
plt.xticks(rotation=45, ha="right")

# Add labels and title
plt.xlabel("Proposal with Date")
plt.ylabel("USD Proposed Value")
plt.title("USD Proposed Value by Proposal Date")
